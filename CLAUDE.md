...
1. **Keepa MCP-сервер** (`src/keepa_mcp/`) — основной интерактивный инструмент.
   ...
   - `sourcing.py` — ссылки на поиск поставщиков Alibaba по ключевым словам
     из **хвоста** тайтла (продуктовое существительное, а не брендовая линейка;
     `keyword_phrase` режет трейлинг-клаузу после запятой/тире и отбрасывает
     маркетинговые прилагательные) — для отчёта закупщику.
   ...
   - `reports.py` — генерация XLSX (листы «Анализ», «Характеристики», «Сводка» —
     последний с разбивкой по вердиктам, бакетам и рынкам через `_summarize`).
...
Alibaba** (`alibaba_url` по ключевым словам из хвоста тайтла), плюс ссылка на
картинку (в DOCX — встроенная миниатюра, best-effort). И XLSX-лист «Сводка», и
шапка DOCX несут разбивку по **вердиктам, бакетам (с avg momentum) и рынкам**
(`marketplace`). Генераторы —
